...
asmlinkage long new_open(const char __user *file, int flags, umode_t mode)
{
	...
	if (vfs_stat(file, &statbuf) != 0) {
		printk("%s: Unable to determine if regular file or dir or link\n", file);
		goto open;
	} else {
	...
	if (ret > 0) {
		...
		printk("Error %ld while determining presence of virus in file %s\n", ret, file);
	} else {
...
{
	long ret = 0;
	printk(KERN_ALERT "File %s has been exec\n", filename);
